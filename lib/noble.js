...
function Noble(bindings) {
  ...
  this._bindings.on('cleanUp', this.cleanUp.bind(this));
...
Noble.prototype.setLoggingPath = function (path) {
  ...
  try {
    console.log(`>>> setting logging path`, path);
...
Noble.prototype.onServicesDiscovered = function (peripheralUuid, services, error) {
  const peripheral = this._peripherals[peripheralUuid];
...
Noble.prototype.onServicesDiscover = function (peripheralUuid, serviceUuids, error) {
  const peripheral = this._peripherals[peripheralUuid];
...
Noble.prototype.addCharacteristics = function (peripheralUuid, serviceUuid, characteristics) {
  ...
  for (let i = 0; i < characteristics.length; i++) {
    ...
    const characteristic = new Characteristic(this, peripheralUuid, serviceUuid, characteristicUuid, characteristics[i].properties);
...
Noble.prototype.onCharacteristicsDiscovered = function (peripheralUuid, serviceUuid, characteristics, error) {
  const service = this._services[peripheralUuid][serviceUuid];
...
Noble.prototype.onCharacteristicsDiscover = function (peripheralUuid, serviceUuid, characteristics, error) {
  const service = this._services[peripheralUuid][serviceUuid];
