...
const discoverServices = function (uuids, callback) {
  if (callback) {
    this.once('servicesDiscover', (services, error) => {
      ...
      service.discoverCharacteristics(characteristicsUuids, (error, characteristics) => {
        ...
        if (error == null) {
          for (const j in characteristics) {
            allCharacteristics.push({ ...characteristics[j], service });
          }
        } else {
...
Peripheral.prototype.discoverSomeServicesAndCharacteristicsAsync = function (serviceUuids, characteristicsUuids) {
  return new Promise((resolve, reject) =>
    this.discoverSomeServicesAndCharacteristics(serviceUuids, characteristicsUuids, (error, services, characteristics) =>
      error
        ? reject(error)
        : resolve({
            services,
...
Peripheral.prototype.discoverAllServicesAndCharacteristicsAsync = function () {
  return new Promise((resolve, reject) =>
    this.discoverAllServicesAndCharacteristics((error, services, characteristics) =>
      error
        ? reject(error)
        : resolve({
            services,
