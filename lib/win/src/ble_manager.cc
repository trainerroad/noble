...
template <typename T>
auto inFilter(std::vector<T> filter, T object)
{
    return filter.empty() || std::find(filter.begin(), filter.end(), object) != filter.end();
}

template <typename O, typename M, class... Types>
auto bind2(O *object, M method, Types &...args)
{
...
#define CHECK_DEVICE()                                     \
    ...
    {                                                      \
        ...
        LOGE("++ CHECK_DEVICE return false");              \
        return false;                                      \
    }
                             ...
                             const std::string uuid)
{
    ...
    if (status == AsyncStatus::Completed)
    {
        ...
        {
            LOGE("++ OnConnected result is null");
...
{
    LOGE("++ OnConnectionStatusChanged");
    ...
    LOGE("++ OnConnectionStatusChanged return");
}
...
void BLEManager::OnIncludedServicesDiscovered(IAsyncOperation<GattDeviceServicesResult> asyncOp,
                                              ...
                                              const std::vector<winrt::guid> serviceUUIDs)
{
    LOGE("++ OnIncludedServicesDiscovered");
    if (status == AsyncStatus::Completed)
    {
...
void BLEManager::OnCharacteristicsDiscovered(IAsyncOperation<GattCharacteristicsResult> asyncOp,
                                             ...
                                             const std::vector<winrt::guid> characteristicUUIDs)
{
    LOGE("++ OnCharacteristicsDiscovered");
    ...
    if (status == AsyncStatus::Completed)
    {
        ...
        {
            LOGE("communication statusxy: %d", resultStatus);
