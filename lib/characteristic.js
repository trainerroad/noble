...
const read = function (callback) {
  ...
  this._noble.read(this._peripheralId, this.serviceUuid, this.uuid);
};
...
const write = function (data, withoutResponse, callback) {
  if (process.title !== 'browser') {
    const allowedTypes = [Buffer, Uint8Array, Uint16Array, Uint32Array];
  ...
  this._noble.write(this._peripheralId, this.serviceUuid, this.uuid, data, withoutResponse);
};
...
const broadcast = function (broadcast, callback) {
  ...
  this._noble.broadcast(this._peripheralId, this.serviceUuid, this.uuid, broadcast);
};
...
const notify = function (notify, callback) {
  ...
  this._noble.notify(this._peripheralId, this.serviceUuid, this.uuid, notify);
};
...
const discoverDescriptors = function (callback) {
  ...
  this._noble.discoverDescriptors(this._peripheralId, this.serviceUuid, this.uuid);
};
